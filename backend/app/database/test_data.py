...
def insert_initial_data(db: Session):
    ...
    if db.query(User).count() > 0:
        print("➡️ Databasen är redan fylld med testdata. Hoppar över insättning.")
        return

    # --- 1. SKAPA ATTRIBUT ---
    attr_taktisk = Attribute(name="Taktisk", description="Förmåga att planera.")
    attr_lojal = Attribute(name="Lojalitet", description="Pålitlighet och trogenhet.")
    
    db.add_all([attr_taktisk, attr_lojal])
    db.flush() # Måste köras för att få ID:n innan de kan användas i association tables
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n❌ FATALT FEL: Databasanslutningen misslyckades vid insättning: {e}")
